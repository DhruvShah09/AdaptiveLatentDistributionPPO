...
def compute_state_visitation(env_states, resolution=10):
    ...
    """
    # Ensure `env_states` is a list of NumPy arrays
    env_states = [(state[0].cpu().numpy(), state[1].cpu().numpy()) if isinstance(state[0], torch.Tensor) else state for state in env_states]
    x_coords, y_coords = zip(*env_states)  # Separate x and y coordinates

    # Convert to NumPy arrays for np.histogram2d
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            state_x = next_obs[:, 0].mean().cpu().numpy()
            state_y = next_obs[:, 1].mean().cpu().numpy()
            
            visited_states.append((state_x, state_y))
            unique_states.add((float(state_x), float(state_y)))
        ...
        if update % visualization_interval == 0:
            ...
            state_diversity_save_path = f"./results/base_ppo/state_diversity_{update}.npy"
            print(f"Saving state diversity data to: {state_diversity_save_path}")
            np.save(state_diversity_save_path, heatmap)

            coverage_data_save_path = f"./results/base_ppo/coverage_over_time_{update}.npy"
